CREATE SCHEMA BancoCC3M;
USE BancoCC3M;

/* CONSULTA 1: retorne o nome e sobrenome de todos os administradores (officer) com o nome
...
/*CONSULTA 2: retorne os nome dos clientes (nome das pessoas jurídicas ou nome + sobrenome das pessoas físicas) 
que possuem uma conta em uma cidade diferente da cidade de estabelecimento.*/

CREATE VIEW consulta2 AS
SELECT CONCAT(i.fname, ' ', i.lname) AS Nome, c.city, br.city AS localEstabelecimento
FROM account a
...
UNION
SELECT b.name AS Nome, c.city, br.city AS localEstabelecimento
FROM account a
JOIN customer c ON a.cust_id = c.cust_id
...
/* CONSULTA 3: retorne os nome de todos os funcionários com, se for o caso, os números de transações 
por ano envolvendo as contas que eles abriram (usando open_emp_id). 
Ordene os resultados por ordem alfabética, e depois por ano (do mais antigo para o mais recente).*/

SELECT CONCAT(e.fname, ' ', e.lname) AS nome, YEAR(t.txn_date) AS ano, COUNT(DISTINCT t.txn_id) AS transacoes
FROM employee e
LEFT JOIN account a ON e.emp_id = a.open_emp_id
LEFT JOIN transaction t ON a.account_id = t.account_id
WHERE t.txn_date IS NOT NULL
GROUP BY e.emp_id, YEAR(t.txn_date)
ORDER BY e.fname, e.lname ASC;

/* CONSULTA 4: retorne os identificadores de contas com maior saldo de dinheiro por agência, 
juntamente com os nomes dos titulares (nome da empresa ou nome e sobrenome da pessoa física) e os nomes dessas agências.*/

CREATE VIEW consulta4 AS 
SELECT a.account_id AS IdConta, CONCAT(i.fname, ' ', i.lname) AS Nome, br.name AS nomeAgencia, a.avail_balance AS saldo
FROM account a
JOIN customer c ON a.cust_id = c.cust_id
JOIN individual i ON c.cust_id = i.cust_id
JOIN branch br ON a.open_branch_id = br.branch_id
WHERE a.avail_balance = (SELECT MAX(a1.avail_balance) FROM account a1 WHERE a1.open_branch_id = a.open_branch_id)
UNION ALL
SELECT a.account_id AS IdConta, b.name AS Nome, br.name AS nomeAgencia, a.avail_balance AS saldo
FROM account a
JOIN customer c ON a.cust_id = c.cust_id
JOIN business b ON c.cust_id = b.cust_id
JOIN branch br ON a.open_branch_id = br.branch_id
WHERE a.avail_balance = (SELECT MAX(a1.avail_balance) FROM account a1 WHERE a1.open_branch_id = a.open_branch_id)
ORDER BY nomeAgencia ASC, saldo DESC;


/* CONSULTA 5: Escreva de novo e modularize as consultas 2. e 4. utilizando uma visualização (CREATE VIEW).*/
...
SELECT officer.fname, officer.lname, business.name
...
SELECT account.account_id, account.cust_id, account.avail_balance, individual.fname, individual.lname, customer.city
...
SELECT MIN(start_date)posse, employee.dept_id, department.name, employee.fname, employee.lname
FROM employee, department
WHERE employee.dept_id = department.dept_id
GROUP BY dept_id, employee.fname, employee.lname;